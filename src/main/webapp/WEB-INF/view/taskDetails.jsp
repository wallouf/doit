				...
				<c:choose>
					<c:when test="${!empty oTask }">
						<p>
							...
							<button onClick="doit_displayTaskStateEditorForDetails(<c:out value="${oTask.id }" />);" type="button" class="btn btn-warning btn-responsive"><span class="glyphicon glyphicon-info-sign"></span> <span class="hidden-xs"><spring:message code="HMI.TASKDETAILS.BUTTON.EDITTASKSTATE" /> </span><spring:message code="HMI.TASK.FORM.PLACEHOLDER.STATE" /></button>
							<a href='<c:url value="/updateTask?taskId=${oTask.id }&sFromUrl=taskDetails?taskId=${oTask.id }" />'   type="button" class="btn btn-warning btn-responsive"><span class="glyphicon glyphicon-pencil"></span> <spring:message code="HMI.GENERIC.BUTTON.EDIT" /><span class="hidden-xs"> <spring:message code="HMI.TASK.FORM.PLACEHOLDER.TASK" /></span></a>
